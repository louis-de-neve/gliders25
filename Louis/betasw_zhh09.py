...
def RInw(Lambda,Tc,S):
    n_air = 1.0 + (5792105.0 / (238.0185 - 1.0 / (Lambda / 1e3)**2) + 167917.0 / (57.362 - 1.0 / (Lambda / 1e3)**2)) / 1e8
    # Refractive index of seawater from Quan and Fry (1994, Applied Optics)
    n0 = 1.31405
    n1 = 1.779e-4
    n2 = -1.05e-6
    n3 = 1.6e-8
...
def dlnasw_ds(Tc, S):
    ...
    a = (-5.58651e-4+2.40452e-7*Tc-3.12165e-9*Tc**2+2.40808e-11*Tc**3)
    b = 1.5*(1.79613e-5-9.9422e-8*Tc+2.08919e-9*Tc**2-1.39872e-11*Tc**3)*S**0.5
    c = 2*(-2.31065e-6-1.37674e-9*Tc-1.93316e-11*Tc**2)*S
...
def betasw_ZHH2009(Lambda,Tc,theta,S,delta):
    ...
    Lambda = np.full_like(Tk, Lambda)#'; # a row variable
    rad = np.full_like(Tk, theta)*np.pi/180; # angle in radian as a colum variable
    ...
    #print(np.shape(beta90sw), "here")
    betasw = beta90sw*(1+((np.cos(rad))**2)*(1-delta)/(1+delta))
    betasw = np.asarray(betasw)
    #print(np.shape(betasw), "here2")
