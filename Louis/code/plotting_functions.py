from setup.setup import Profile, Transect, two_dimensional_binning, import_split_and_make_transects
...
def new_binned_plot(valid_profiles:list[Profile], ax:plt.axes, parameter:str, bin_size:float=2, max_depth:float=1000., **kwargs) -> mpl.collections.PolyQuadMesh:
    ...
    for i, p in enumerate(valid_profiles):
        ...
        p.valid_next = "n"
        if i < len(valid_profiles)-1 and valid_profiles[i+1].index - p.index == 2:
        ...
        if parameter in ["chlorophyll", "chlorophyll_corrected"]:
            pcm = ax.pcolormesh(X, Y, data, norm=mpl.colors.LogNorm(vmin=0.01, vmax=36))
        else:
