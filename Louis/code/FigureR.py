...
ax1 = fig.add_axes([0.2, 0.13, 0.75, 0.85])
...
p3 = temp_salinity_plot(boundary, ax1, colors[2])
...
X, Y = np.meshgrid(
    xs,
    np.linspace(ylims[0], ylims[1], 1000)
)
sigma = sigma0(X, Y) + 1000
CS = ax1.contour(X, Y, sigma, colors="black", linewidths=0.5, levels=(1027.15, 1027.3, 1027.45, 1027.6, 1027.75, 1027.9))

ax1.clabel(CS, fontsize=6)

ax1.scatter([0], [0], c="#00000000", marker="o", edgecolor=colors[0], linewidth=1.5, s=20, label="Interior")
ax1.scatter([0], [0], c="#00000000", marker="o", edgecolor=colors[1], linewidth=1.5, s=20, label="Edge")
ax1.scatter([0], [0], c="#00000000", marker="o", edgecolor=colors[2], linewidth=1.5, s=20, label="Boundary")
