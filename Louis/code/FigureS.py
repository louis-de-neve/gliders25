...
profiles = [p for p in profiles if p.index not in [595, 594]]
...
print(exterior1[0].start_location[1], exterior1[0].start_location[0])
print(exterior2[-1].end_location[1], exterior2[-1].end_location[0])
print(interior1[0].start_location[1], interior1[0].start_location[0])
...
for profiles, loop_ax, loop_ax1, loop_ax2 in [(profiles1, ax, ax1, ax2), (profiles2, axb, ax1b, ax2b)]:
    ...
    my_cmap = cmocean.cm.thermal.copy()
    my_cmap.set_extremes(over=(0,0,0), under=(1,1,1))
    my_norm = mpl.colors.Normalize(vmin=-1.5, vmax=1.5, clip=True)

    pcm = new_binned_plot(profiles, loop_ax, "CT", 3, 800, cmap=my_cmap, norm=my_norm)
    c1 = plt.colorbar(pcm, cax=cbar_ax)
    cbar_ax.set_ylabel(r"Temperature / °C")


    my_cmap = cmocean.cm.haline.copy()
    # Add a section of the RdPu colormap to the lower portion of this cmap

    # Get the original haline colormap as an array
    haline_colors = my_cmap(np.linspace(0, 1, 256))[15:]
    rdpu = plt.get_cmap('PiYG_r')
    rdpu_section = rdpu(np.linspace(0.6, 1, 25))
    combined_colors = np.vstack([rdpu_section, haline_colors])

    # Create new colormap
    my_cmap = ListedColormap(combined_colors)
    my_cmap.set_extremes(over=(0,0,0), under=(1,1,1))
    my_norm = mpl.colors.Normalize(vmin=34, vmax=34.8, clip=True)

    pcm2 = new_binned_plot(profiles, loop_ax1, "SA", 3, 800, cmap=my_cmap, norm=my_norm)
    c2 = plt.colorbar(pcm2, cax=cbar_ax2)
    cbar_ax2.set_ylabel(r"Salinity / g kg$^{-1}$")
...
#fig.tight_layout()
plt.savefig("Louis/figures/figureS.png", dpi=300)
