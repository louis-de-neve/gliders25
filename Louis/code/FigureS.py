...
for profiles, loop_ax, loop_ax1, loop_ax2 in [(profiles1, ax, ax1, ax2), (profiles2, axb, ax1b, ax2b)]:
    ...
    pcm = new_binned_plot(profiles, loop_ax, "temperature_final", 3, 800, cmap=my_cmap, norm=my_norm)
    ...
    my_cmap.set_extremes(over=(0,0,0), under=(1,1,1))
    my_norm = mpl.colors.LogNorm(vmin=33.8, vmax=34.7, clip=True)

    pcm2 = new_binned_plot(profiles, loop_ax1, "salinity_final", 3, 800, cmap=my_cmap, norm=my_norm)
    c2 = plt.colorbar(pcm2, cax=cbar_ax2)
    cbar_ax2.set_ylabel(r"Salinity / g/kg")
